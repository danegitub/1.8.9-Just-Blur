...
public class FaithfulBlurRenderer {
    ...
    public void render() {
        ...
        if (texA == -1 || texB == -1 || width != w || height != h) {
            deleteTextures();

            texA = GlStateManager.generateTexture();
            texB = GlStateManager.generateTexture();
            width = w;
            height = h;

            setup(texA, width, height);
            setup(texB, width, height);
        ...
        double strength = getStrength(motion, cameraDelta);
        if (strength < 0.02D) {
        ...
        draw(texA, strength * 0.85D);
        draw(texB, strength * 0.25D);
    ...
    private static void deleteTextures() {
        if (texA != -1) {
            GlStateManager.deleteTexture(texA);
            texA = -1;
        }

        if (texB != -1) {
            GlStateManager.deleteTexture(texB);
            texB = -1;
        }
    }
    ...
    private static void copy(int tex) {
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, tex);
    ...
    private static double getStrength(double motion, float cameraDelta) {
        ...
        if (MotionBlurMod.config.adaptive) {
            ...
            double moveFactor = Math.min(1.0D, motion * 10.0D);
            double cameraFactor = MotionBlurMod.config.cameraBased ? Math.min(1.0D, cameraDelta / 30.0D) : 0.0D;

            strength = strength * (0.35D + moveFactor * 0.40D + cameraFactor * 0.25D) * fpsFactor;
        } else if (MotionBlurMod.config.cameraBased) {
            strength = strength * (0.70D + Math.min(1.0D, cameraDelta / 30.0D) * 0.30D);
        }
        ...
        return clamp(strength, 0.0D, 0.78D);
    }
