...
public class FaithfulBlurRenderer {
    ...
    public static void render() {
        ...
        if (texA == -1 || texB == -1 || w != width || h != height) {
        ...
        double s = getAdaptive(MotionBlurMod.config.strength);
        s = clamp(s, 0.0, 0.75);

        double strong = s * 0.85;
        double light = s * 0.25;
    ...
    private static double getAdaptive(double base) {
        ...
        double moveFactor = Math.min(1.0, motion * 10.0);

        return base * (0.4 + moveFactor * 0.6) * fpsFactor;
    }
